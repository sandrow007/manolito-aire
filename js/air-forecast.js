...
async function fetchAirSeries(lat, lon) {
	...
	try {
		...
		/* almacenamiento no disponible: se pide a la red */ }
	...
	const salida = {
		times,
		values,
		nowIndex: nowIndex === -1 ? Math.floor(times.length / 2) : nowIndex
	};
	try {
...
function drawAirChart(times, values, nowIndex) {
    ...
    <svg viewBox="0 0 ${w} ${h}" preserveAspectRatio="xMidYMid meet" role="img"
      ...
      <g class="chart-cursor" style="display:none">
        <line class="chart-cursor-line" x1="0" y1="${padT}" x2="0" y2="${h-padB}" />
        <circle class="chart-cursor-dot" cx="0" cy="0" r="4.5" />
      </g>
    </svg>
    <div class="chart-tip" role="status" aria-live="polite"></div>
  `;

	/* ----- Interactividad: recorrer la línea con dedo, ratón o teclado -----
	   Un dedo/ratón sobre la gráfica muestra la hora y el valor exacto de
	   PM2.5; con teclado se recorre con las flechas izquierda/derecha.
	   El punto se tiñe del color del semáforo del aire (verde/ámbar/rojo). */
	const svg = el.querySelector('svg');
	const cursor = el.querySelector('.chart-cursor');
	const cursorLine = el.querySelector('.chart-cursor-line');
	const cursorDot = el.querySelector('.chart-cursor-dot');
	...
	let indiceFijado = -1; // con teclado queda "fijado" hasta Escape o salir

	const mostrarPunto = (i) => {
		...
		const pct = (px / w) * 100;
		tip.style.left = `clamp(8%, ${pct}%, 92%)`;
	};
	const ocultarPunto = () => {
		if (indiceFijado >= 0) return; // fijado por teclado: no se apaga
		cursor.style.display = 'none';
		tip.classList.remove('visible');
	};

	const indiceDesdeEvento = (e) => {
		const rect = svg.getBoundingClientRect();
		if (!rect.width) return -1;
		const pxViewBox = ((e.clientX - rect.left) / rect.width) * w;
		return Math.round(((pxViewBox - padL) / (w - padL - padR)) * (n - 1));
	};

	svg.addEventListener('pointermove', (e) => {
		const i = indiceDesdeEvento(e);
		if (i >= 0) mostrarPunto(i);
	});
	...
	svg.addEventListener('pointerleave', ocultarPunto);
	svg.addEventListener('keydown', (e) => {
		if (e.key === 'ArrowLeft' || e.key === 'ArrowRight') {
			e.preventDefault();
			const paso = e.key === 'ArrowLeft' ? -1 : 1;
			indiceFijado = Math.max(0, Math.min(n - 1, (indiceFijado < 0 ? nowIndex : indiceFijado) + paso));
			mostrarPunto(indiceFijado);
...
function quantumAirModel(forecastValues) {
	...
	let amplitudes = bins.map((b, i) => {
		...
		return {
			re: real * Math.cos(phase),
			im: real * Math.sin(phase)
		};
	});
...
function renderQuantumBars(result) {
  ...
  `).join('');
	...
	const ocultarSiNoFijado = () => {
		if (!el.querySelector('.qbar-activo')) {
			detail.textContent = '';
	...
	el.querySelectorAll('.qbar-btn').forEach((btn) => {
		...
		btn.addEventListener('click', () => {
			...
			el.querySelectorAll('.qbar-btn').forEach(b => {
				b.classList.remove('qbar-activo');
...
async function loadForecastForCity() {
	...
	try {
		const {
			...
			values,
			nowIndex
		} = await fetchAirSeriesWithRetry(d.lat, d.lon);
