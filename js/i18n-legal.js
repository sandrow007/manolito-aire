...
(function () {
  ...
  const en = {
    ...
    avis_36: '12. Pedestrian Route Calculation: the Thermal Dijkstra Algorithm',
    avis_37: 'The route finder in the "3D Shade Route" section does not pick the shortest street, but the coolest one: it applies the classic <strong>Dijkstra algorithm</strong> over a <b>pedestrian graph</b> built from <strong>OpenStreetMap</strong> streets (a preloaded local network, filtered to roughly a 500&nbsp;m radius around the trip, with on-demand expansion via the Overpass API for longer journeys). Manolit∞ Aire\'s twist is that <b>each segment\'s weight is not just its length</b>: every edge in the graph is weighted with the formula <code>w(e) = length (m) × (1 + solar penalty)</code>, where the penalty is computed at the segment\'s midpoint by crossing the <strong>real position of the sun</strong> (altitude and azimuth, using standard SunCalc astronomical algorithms) with the shadows cast by buildings and trees at that very moment. Walking in full sun therefore "costs" up to 70% more than walking in shade, so the algorithm returns the path of <b>lowest total thermal cost</b>, not the shortest one in meters.',
    avis_38: 'All computation runs <b>entirely in your browser</b>, using a custom JavaScript implementation with a from-scratch binary priority queue (<i>min-heap</i>): your origin and destination positions are not sent to any routing server for this calculation. The approach is inspired by academic work from the <strong>University of Seville</strong> on urban shade maps and routes. The result depends on OpenStreetMap coverage (building heights and tree data are estimated when records are missing) and on a software-computed solar position, so the suggested route is <b>indicative and educational</b>: it is not technical, urban-planning or health advice, and during extreme heat events official recommendations must always be followed.',
    avis_34: '13. The origin of Manolit∞: Survival in Seville',
